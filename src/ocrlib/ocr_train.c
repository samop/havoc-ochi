/* ocr_train.c -- command line application to train ocr network
 * copyright (c) 2003 Peter van Rossum
 * $Id: ocr_train.c,v 1.7 2003/10/23 17:46:28 petervr Exp $ */
...
int batch = 0;                  /* switch --batch, -b */
int create_network = 0;         /* switch --create-network, -c */
int hidden_nodes = 10;          /* switch --hidden-nodes, -n */
float max_error = 0.01;         /* switch --error, -e */
int max_trainings = 10000000;   /* switch --trainings, -t */
int stop_err_inc = 0;           /* switch --first-bump, -f */
